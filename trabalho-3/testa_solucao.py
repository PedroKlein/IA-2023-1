...
class TestaSolucao(unittest.TestCase):
    def test_funcao_sucessor(self):
        ...
        sucessores = solucao.sucessor(
            "2_3541687"
        )  # obtem os sucessores chamando a funcao implementada
        self.assertEqual(
            3, len(sucessores)
        )  # verifica se foram retornados 3 sucessores
        for (
            s
        ) in (
            sucessores
        ):  # verifica se os sucessores retornados estao entre os esperados
    ...
    def test_funcao_expande(self):
        ...
        pai = solucao.Nodo(
            estado_pai, None, "abaixo", 2
        )  # o pai do pai esta incorreto, mas nao interfere no teste
        ...
        resposta = solucao.expande(
            pai
        )  # obtem a resposta chamando a funcao implementada
        ...
        for nodo in resposta:
            ...
            self.assertIn(
                (nodo.estado, nodo.pai.estado, nodo.acao, nodo.custo), resposta_esperada
            )
    ...
    def test_run_astar_hamming(self):
        ...
        self.assertEqual(
            23, len(self.run_algorithm(solucao.astar_hamming, "2_3541687"))
        )
    ...
    def test_run_astar_manhattan(self):
        ...
        # no estado 2_3541687, a solucao otima tem 23 movimentos.
        self.assertEqual(
            23, len(self.run_algorithm(solucao.astar_manhattan, "2_3541687"))
        )

        # nao ha solucao a partir do estado 185423_67
        self.assertIsNone(self.run_algorithm(solucao.astar_manhattan, "185423_67"))

    def test_run_astar_euclidiana(self):
        """
        Testa o A* com dist. Euclidiana em um estado com solução e outro sem solução.
        Atencao! Passar nesse teste com '2_3541687' apenas significa que a lista retornada tem o
        numero correto de elementos. O teste nao checa se as acoes levam para a solucao!
        :return:
        """
        # no estado 2_3541687, a solucao otima tem 23 movimentos.
        self.assertEqual(
            23, len(self.run_algorithm(solucao.astar_new_heuristic, "2_3541687"))
        )
